...
稽核時間：2026-08-23（Asia/Taipei；本輪本機與正式包已重驗，公開部署將於本輪推送後重驗）
...
| P0-02 | 已完成／待公開驗證 | `09_HTML動態簡報` 101 頁內容可讀、長標題不裁切、長文字可捲動、桌機／手機／平板 RWD；操作型頁面改為語意化步驟／完成面板 | 本機 `node qa_github_pages_site.mjs` → exit 0；總覽 101 卡片／101 直達連結；上午 scenes=282、capability=282，下午 scenes=324、capability=324；兩場垂直捲動探針 viewports=6、passed=6；兩場捲動重設 maxResidual=0；桌機水平溢位=0；正式包 HTML QA → exit 0 | 公開部署後補跑 `BASE_URL` QA；後續不得以固定高度或隱藏溢位方式回退內容可讀性 |
| P0-03 | 已完成／待公開驗證 | Service Worker 更新提示、版本快取與 GitHub Pages 發布 | 本機 `node sync_dynamic_deck.mjs` → exit 0、version=`2026.08.23.05`；發布前尚未確認本輪 Pages workflow、`status=built` 與公開 `version.json` | 推送後必須重新確認 Pages `built`、`version.json` HTTP 200 與公開 `BASE_URL` QA |
...
| P1-03 | 已完成／待公開驗證 | 專區 favicon、PWA icon／manifest、LINE／Facebook／Twitter OG 預覽與公開 meta | 本機 `node qa_github_pages_site.mjs` → exit 0；總覽與 4 個動態頁的 meta／favicon／manifest 與資產契約均通過；版本 `.05`、OG PNG=1200×630；公開版本未確認 | 公開部署後補跑 `BASE_URL` QA；若要增加多組分享卡或動態預覽，另列候選，不改既有分享圖契約 |
| P1-04 | 基線完成／待公開驗證，仍有維護缺口 | QA 已涵蓋 encoded 中文路徑、短高度桌機、RWD、內層捲動、換頁捲動歸零、跨場總覽與公開站；QA 腳本仍在專案外層 | `node --check qa_github_pages_site.mjs` → exit 0；本機上午／下午捲動容器 282/282、324/324，探針與重設各 6/6、maxResidual=0；總覽卡片 101 張、直接連結 101 個、RWD 3 種尺寸；公開版本未確認；本機工具標籤 named=72、numeric=0 | QA 腳本仍不在 Pages site repo，版控狀態需另行決定；可選 P1-E |
...
| P1-06 | 已完成／待公開驗證 | 901px 以上橫向全螢幕的內容放大、可用寬度釋放、長文垂直捲動與大型觸控螢幕相容 | `node qa_fullscreen_dynamic.mjs`（FULLSCREEN_ALL=1）→ exit 0；上午 47/47、下午 54/54，全螢幕舞台 1912×1078、水平溢位 0；第 4 頁標題 84.128px、說明 24.856px；本機 CSS 單一動畫控制規則存在 | 僅改 `09_HTML動態簡報` CSS 最末層；不得回退既有手機／平板 RWD、長文捲動、轉場或 `08_HTML簡報` 圖檔架構 |
| P1-07 | 已完成／待公開驗證 | 跨上午／下午的 HTML 簡報總覽專區、場次篩選、關鍵字搜尋與指定頁直接跳轉 | `node qa_github_pages_site.mjs` → exit 0；總覽卡片 101 張（上午 47／下午 54）、直接連結 101 個、篩選按鈕 3 個；下午篩選 54 張；`Codex` 搜尋命中 32 張；RWD 393×852、932×430、820×1180 共 3 種尺寸均無水平溢位 | 維持 `overview.html` 為跨場索引；單場頁的 `O` 快速總覽保留，不互相取代 |
...
| P1-09 | 已完成／待公開驗證 | 操作型 HTML 投影片右側卡片 UI／UX：步驟、完成條件、編號、勾選、時間標籤與連結分層；避免最高卡片撐高同列空白 | `node --check github_pages_site/09_HTML動態簡報/assets/dynamic-deck.js` → exit 0；全螢幕上午 47/47、下午 54/54；HyperFrames `check` → exit 0、lint/runtime/layout/motion 問題 0、contrast=99/99；正式包 HTML QA → exit 0；公開 QA 未確認 | 新增操作型頁面時優先提供清楚的步驟／驗收資料；不回寫 `08_HTML簡報` 圖檔內容 |
| P1-10 | 已完成／待公開驗證 | 全螢幕 HTML 互動轉場單一動畫控制：外層雙緩衝不再與 GSAP 重複淡入／模糊／縮放，保留內容分層進場 | `node qa_fullscreen_animation.mjs` → exit 0；1912×1078；上午／下午前進與後退共 4 個方向、16 個取樣、failures=0、pageErrors=0；`node qa_fullscreen_dynamic.mjs`（FULLSCREEN_ALL=1）→ exit 0；上午 47/47、下午 54/54、水平溢位=0；本機 CSS 單一動畫控制規則存在；`08_HTML簡報` 未改投影片圖片 | 公開部署後補跑動畫與 CSS HTTP 200 複核；後續若要增加轉場效果，必須先通過全螢幕動畫 QA，不得讓投影片外層與內容層同時控制同一個 opacity／filter／transform |
| P1-11 | 已完成／待公開驗證 | 上午開場「降本增效」與下午 Human-in-the-loop 教學內容 | `node build_decks.mjs` → exit 0；PPTX 上午 47 頁、下午 54 頁；`node build_html_deck.mjs` → exit 0；HTML morning=47、afternoon=54；`node sync_dynamic_deck.mjs` → exit 0、version=`2026.08.23.05`；公開 `BASE_URL` QA 未確認 | 上午以開場標題／章節帶入「降本增效」；下午 Human-in-the-loop 實際仍為第 7 頁；圖檔版與 HTML 版均由同一來源重建 |
...
| P1-13 | 已完成／待公開驗證 | 上午／下午 HTML 案例工具連結改用實際工具名稱，並區分「工具卡片」與「開啟應用」 | `node build_html_deck.mjs` → exit 0；HTML morning=47、afternoon=54；`node qa_html_deck.mjs` → exit 0；named=12+12、numeric=0；本機 `node qa_github_pages_site.mjs` → exit 0；工具標籤 named=72、numeric=0；靜態與動態 HTML 均同步；公開檔案複核未確認 | 公開部署後補跑公開工具標籤與檔案複核；不改 `08_HTML簡報` 圖檔、Hotspot、QR；未來新增案例沿用實際工具名稱，不再以編號作為唯一可見標籤 |
| P1-14 | 已完成／待公開驗證 | 上午開場新增 Vibe Coding、Vibe Working 與 AI 三個 Level，並把上午 Level 1／2 連到下午 Level 3 Agentic AI | `node build_decks.mjs` → exit 0；新增上午第 2–4 頁；`node build_html_deck.mjs`／`node sync_dynamic_deck.mjs` → exit 0；上午 47 頁、版本 `.05`；本機 `node qa_html_deck.mjs`、`node qa_github_pages_site.mjs`、正式包 QA 均 exit 0 | Vibe Working 已標註為本工作坊延伸框架；公開部署後確認新頁文字、直接連結與頁碼均可用 |
