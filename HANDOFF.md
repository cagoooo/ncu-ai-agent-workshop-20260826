...
- 名稱：`換AGENT繼續加油`
- 分類：`開發流程`
- 內容已完整還原為使用者直接貼上的本地端原始版本：第一行使用小寫 `agent` 與半形斜線 `/`（`Claude Code / Codex / Antigravity`），並保留所有原文條列、`未確認` 要求、讀取 `AGENTS.md` 與 `HANDOFF.md` 的指示，以及最後一句「最後請提供給我接手的AGENT的提示詞」；此段是面板內容，不是本輪立即執行指令。
...
```text
正式包同步（排除 .git）→ exit 0；copied_root_items=23、formal_git_before=False、formal_git_after=False
正式包 node qa_html_deck.mjs → exit 0；上午 named=12、numeric=0；下午 named=12、numeric=0；HTML deck QA passed
正式包 node audit_local_links.mjs → exit 0；87 HTML files、183 local references
正式包 node qa_workshop_suite.mjs → exit 0；Workshop suite QA passed
python -X utf8 rebuild_release_manifest.py <正式包> → exit 0；total=500、inventory=499、pdf=6、duplicate_pdf_groups=2
git commit -m "優化 HTML 簡報資源列多列排版" → exit 0；commit=b1b6801；12 files changed
git push origin main → exit 0；d1f88ea..b1b6801，main → main
gh run watch 32621002147 --repo cagoooo/ncu-ai-agent-workshop-20260826 --exit-status → exit 0；build=97149038647、report=97149063891、deploy=97149063899 全部成功（僅有既有 Node.js 20 deprecation annotation）
gh api repos/cagoooo/ncu-ai-agent-workshop-20260826/pages --jq '{status:.status,url:.html_url}' → exit 0；status=built
Invoke-WebRequest version.json?cb=20260823-08-final → exit 0；HTTP 200、570 bytes、公開 version=2026.08.23.08、title=資源列多列化與完整文字區塊避讓
Invoke-WebRequest 公開 dynamic-deck.css?v=2026.08.23.08 → exit 0；HTTP 200、56176 bytes；flex-wrap=wrap、overflow=visible、position=relative !important 均存在
Invoke-WebRequest 公開 morning.html?cb=20260823-08-final → exit 0；HTTP 200、6857 bytes；版本 query=2026.08.23.08
$env:BASE_URL='https://cagoooo.github.io/ncu-ai-agent-workshop-20260826'; node qa_github_pages_site.mjs; Remove-Item Env:BASE_URL → exit 0；公開總覽 101 卡片／101 直達連結／3 篩選器／下午 54 張；轉場 samples=2；上午／下午 scenes=282／324、capability=282／324；垂直捲動各 6/6；捲動重設各 6/6、maxResidual=0；公開工具標籤 named=72、numeric=0；資源列換行／可見／不重疊斷言全數通過
```
...
- Vibe Working 頁明確標註為本工作坊延伸的工作框架，不把它誤寫成 Karpathy 原始名詞；內容從看見痛點、捕捉靈感、描述工作，到讓 Agent 推進並把時間還給人。
- AI 三個 Level 對應上午到下午的課程路徑：Level 1 對話式 AI、Level 2 協作式 AI（Gemini Notebook／Canvas）、Level 3 Agentic AI／AI 代理人（長時間執行、Skills 與人類決策）。
- 圖檔／PPTX 版與 HTML 版均由同一套來源重建並同步；原有圖檔版、Hotspot、QR、雙緩衝、全螢幕 CSS SSOT 與 RWD 沒有被 HTML 專區取代；本輪沒有輸出 MP4。
- 前一輪已完成的 OpenClaw、Gemini 3.7 Flash 與「Antigravity／Gemini 3.7 Flash 速度快 20 倍以上」個人工作流體感均保留，沒有刪除或改寫。
...
node build_decks.mjs → exit 0；PPTX／圖檔版上午 47 頁、下午 54 頁，共 101 頁
node build_html_deck.mjs → exit 0；HTML decks exported: morning=47, afternoon=54
node sync_dynamic_deck.mjs → exit 0；morning=47、afternoon=54、version=2026.08.23.05
node qa_html_deck.mjs → exit 0；上午 named=12、numeric=0；下午 named=12、numeric=0；HTML deck QA passed
node qa_github_pages_site.mjs → exit 0；總覽 101 卡片／101 直達連結／3 篩選器／下午 54 張；上午 scenes／capability=282／282，下午=324／324；垂直捲動各 6/6；捲動重設各 6/6、maxResidual=0；公開工具標籤 named=72、numeric=0
python -X utf8 qa_qr_codes.py → exit 0；151 rendered QR codes decoded
npx --yes hyperframes check "github_pages_site/09_HTML動態簡報" --json → exit 0；ok=true、filesScanned=3、lint/runtime/layout/motion 問題 0、contrast=99/99
node qa_fullscreen_dynamic.mjs（FULLSCREEN_ALL=1；上午）→ exit 0；47/47、failures=0、horizontalOverflow=0
node qa_fullscreen_dynamic.mjs（FULLSCREEN_ALL=1；下午）→ exit 0；54/54、failures=0、horizontalOverflow=0
node qa_fullscreen_animation.mjs → exit 0；4 個方向、16 個取樣、failures=0、pageErrors=0
正式包 qa_html_deck.mjs／audit_local_links.mjs／qa_workshop_suite.mjs → exit 0；HTML deck QA passed、86 HTML files／176 local references、Workshop suite QA passed
python -X utf8 rebuild_release_manifest.py <正式包> → exit 0；total=485、inventory=484、pdf=6、duplicate_pdf_groups=2
git commit -m "上午新增 Vibe Coding 與 AI 三個 Level" → exit 0；103bb49；114 files changed
git push origin main → exit 0；72798e2..103bb49，main → main
gh run watch 32613743605 --repo cagoooo/ncu-ai-agent-workshop-20260826 --exit-status → exit 0；build=97130849437、report=97130873740、deploy=97130873742 全部成功（僅有 Node.js 20 deprecation annotation）
gh api repos/cagoooo/ncu-ai-agent-workshop-20260826/pages --jq '{status:.status,url:.html_url}' → exit 0；status=built
Invoke-WebRequest version.json?cb=20260823-05-final → exit 0；HTTP 200、561 bytes、公開 version=2026.08.23.05、title=上午新增 Vibe Coding、Vibe Working 與 AI 三個 Level
$env:BASE_URL='https://cagoooo.github.io/ncu-ai-agent-workshop-20260826'; node qa_github_pages_site.mjs; Remove-Item Env:BASE_URL → exit 0；公開總覽 101 卡片／101 直達連結／3 篩選器／下午 54 張；上午／下午 scenes=282／324、capability=282／324；垂直捲動各 6/6；捲動重設各 6/6、maxResidual=0；公開工具標籤 named=72、numeric=0
公開檔案複核 → exit 0；7 個端點均 HTTP 200：version=561 bytes、08 morning=111620、08 afternoon=122631、dynamic morning data=83306、dynamic afternoon data=94866、overview=5862、dynamic CSS=54069；公開上午資料含 Vibe Coding／Vibe Working／AI 三個 Level
```

## 前一輪已完成：HTML 工具卡片改用實際工具名稱（v2026.08.23.04）
...
git commit -m "更新 HTML 工具卡片實際名稱" → exit 0；2afc44b；15 files changed、120 insertions、107 deletions
git push origin main → exit 0；f8e2669..2afc44b，main → main
gh run watch 32608869880 --repo cagoooo/ncu-ai-agent-workshop-20260826 --exit-status → exit 0；build=97118404459、deploy=97118433720、report=97118433775 全部成功（僅有 Node.js 20 deprecation annotation）
gh api repos/cagoooo/ncu-ai-agent-workshop-20260826/pages --jq '{status:.status,url:.html_url}' → exit 0；status=built
Invoke-WebRequest version.json?cb=20260823-04-live → exit 0；HTTP 200、519 bytes、公開 version=2026.08.23.04、title=HTML 工具卡片改用實際工具名稱
$env:BASE_URL='https://cagoooo.github.io/ncu-ai-agent-workshop-20260826'; node qa_github_pages_site.mjs; Remove-Item Env:BASE_URL → exit 0；公開總覽 98 卡片／98 直達連結／3 篩選器／下午 54 張；上午／下午 scenes=264／324、capability=264／324；垂直捲動各 6/6；捲動重設各 6/6、maxResidual=0；公開工具標籤 named=72、numeric=0
公開檔案複核 → exit 0；`08_HTML簡報/morning.html` HTTP 200、103174 bytes；`08_HTML簡報/afternoon.html` HTTP 200、122631 bytes；動態 morning／afternoon data HTTP 200、75972／94866 bytes；公開 CSS HTTP 200、54069 bytes；`sw.js` HTTP 200、5047 bytes；公開工具名稱 named=96、numeric=0、required_names=4、missing_names=0
```
...
|---|---|---|---|---|
| P0-01 | 已完成／守門 | `08_HTML簡報` 圖檔版的全螢幕、雙緩衝切換、快取破壞、原有版面與新增教學內容 | `node build_decks.mjs` → exit 0；PPTX／圖檔版上午 47 頁、下午 54 頁，共 101 頁；`node qa_html_deck.mjs` → exit 0；本輪只從 `build_decks.mjs` 來源重建，既有 Hotspot、QR、雙緩衝與全螢幕邏輯保留 | 後續 HTML 動態功能不得改寫圖檔版的圖片、Hotspot、雙緩衝與全螢幕 CSS SSOT；若要再改內容，必須回到簡報來源重建 |
| P0-02 | 已完成／公開可用 | `09_HTML動態簡報` 101 頁內容可讀、長標題不裁切、長文字可捲動、桌機／手機／平板 RWD；操作型頁面改為語意化步驟／完成面板 | 本機與公開 `BASE_URL` 的 `node qa_github_pages_site.mjs` 均 exit 0；總覽 101 卡片／101 直達連結；上午 scenes=282、capability=282，下午 scenes=324、capability=324；兩場垂直捲動探針 viewports=6、passed=6；兩場捲動重設 maxResidual=0；桌機水平溢位=0；正式包 HTML QA → exit 0 | 後續不得以固定高度或隱藏溢位方式回退內容可讀性 |
| P0-03 | 已完成／公開可用 | Service Worker 更新提示、版本快取與 GitHub Pages 發布 | `gh run watch 32613743605 --repo cagoooo/ncu-ai-agent-workshop-20260826 --exit-status` → exit 0；build=97130849437、report=97130873740、deploy=97130873742 全部成功；`gh api repos/cagoooo/ncu-ai-agent-workshop-20260826/pages --jq '{status:.status,url:.html_url}'` → exit 0、`status=built`；`Invoke-WebRequest version.json?cb=20260823-05-final` → exit 0、HTTP 200、561 bytes、公開 version=`2026.08.23.05`；公開 `BASE_URL` QA → exit 0 | 每次發布都要重新確認 Pages `built`、`version.json` HTTP 200 與公開 QA |
...
| P1-01 | 已完成 | HTML 簡報入口、上午／下午場 composition、返回簡報首頁、鍵盤／觸控／單場快速總覽／跨場總覽專區／講者備註／閱讀模式／全螢幕 | `node qa_html_deck.mjs` → exit 0；圖檔版與 HTML 動態版均為上午 47 頁、下午 54 頁，共 101 頁 | 後續可在既有導覽上加功能，不重做場次資料結構 |
...
| P1-03 | 已完成／公開可用 | 專區 favicon、PWA icon／manifest、LINE／Facebook／Twitter OG 預覽與公開 meta | 本機與公開 `BASE_URL` QA → exit 0；總覽與 4 個動態頁的 meta／favicon／manifest 與資產契約均通過；公開 version=`2026.08.23.05`、OG PNG=1200×630；7 個公開端點 HTTP 200 | 若要增加多組分享卡或動態預覽，另列候選，不改既有分享圖契約 |
| P1-04 | 基線完成／公開可用，仍有維護缺口 | QA 已涵蓋 encoded 中文路徑、短高度桌機、RWD、內層捲動、換頁捲動歸零、跨場總覽與公開站；QA 腳本仍在專案外層 | `node --check qa_github_pages_site.mjs` → exit 0；公開上午／下午捲動容器 282/282、324/324，探針與重設各 6/6、maxResidual=0；總覽卡片 101 張、直接連結 101 個、RWD 3 種尺寸；公開版本 `.05`；公開工具標籤 named=72、numeric=0 | QA 腳本仍不在 Pages site repo，版控狀態需另行決定；可選 P1-E |
| P1-05 | 部分完成／仍有決策邊界 | 正式包 Manifest、實際檔案數與重複 PDF 盤點；來源腳本是否納入 Git、PDF structure-tree 可及性仍待決定 | `rebuild_release_manifest.py` → exit 0；正式包總數 485、SHA-256 inventory=484、missing=0、mismatch=0；正式包 PDF 6 份、重複 PDF 群組 2 組；PDF 頁數上午 47、下午 54、合計 101；正式包 `audit_local_links.mjs` → exit 0（86 HTML、176 references）；`qa_workshop_suite.mjs` 與正式包 `qa_html_deck.mjs` 均 exit 0 | 重複 PDF 已依老師決定保留；不刪除原 v1.0 歷史清單；來源納入 Git 與 PDF 完整可及性驗收仍不自行決定 |
| P1-06 | 已完成／公開可用 | 901px 以上橫向全螢幕的內容放大、可用寬度釋放、長文垂直捲動與大型觸控螢幕相容 | `node qa_fullscreen_dynamic.mjs`（FULLSCREEN_ALL=1）→ exit 0；上午 47/47、下午 54/54，全螢幕舞台 1912×1078、水平溢位 0；第 4 頁標題 84.128px、說明 24.856px；公開 CSS HTTP 200、54069 bytes；單一動畫控制規則存在 | 僅改 `09_HTML動態簡報` CSS 最末層；不得回退既有手機／平板 RWD、長文捲動、轉場或 `08_HTML簡報` 圖檔架構 |
| P1-07 | 已完成／公開可用 | 跨上午／下午的 HTML 簡報總覽專區、場次篩選、關鍵字搜尋與指定頁直接跳轉 | 公開 `node qa_github_pages_site.mjs` → exit 0；總覽卡片 101 張（上午 47／下午 54）、直接連結 101 個、篩選按鈕 3 個；下午篩選 54 張；`Codex` 搜尋命中 32 張；RWD 393×852、932×430、820×1180 共 3 種尺寸均無水平溢位 | 維持 `overview.html` 為跨場索引；單場頁的 `O` 快速總覽保留，不互相取代 |
...
| P1-09 | 已完成／公開可用 | 操作型 HTML 投影片右側卡片 UI／UX：步驟、完成條件、編號、勾選、時間標籤與連結分層；避免最高卡片撐高同列空白 | `node --check github_pages_site/09_HTML動態簡報/assets/dynamic-deck.js` → exit 0；全螢幕上午 47/47、下午 54/54；HyperFrames `check` → exit 0、lint/runtime/layout/motion 問題 0、contrast=99/99；正式包 HTML QA → exit 0；公開 QA → exit 0 | 新增操作型頁面時優先提供清楚的步驟／驗收資料；不回寫 `08_HTML簡報` 圖檔內容 |
| P1-10 | 已完成／公開可用 | 全螢幕 HTML 互動轉場單一動畫控制：外層雙緩衝不再與 GSAP 重複淡入／模糊／縮放，保留內容分層進場 | `node qa_fullscreen_animation.mjs` → exit 0；1912×1078；上午／下午前進與後退共 4 個方向、16 個取樣、failures=0、pageErrors=0；`node qa_fullscreen_dynamic.mjs`（FULLSCREEN_ALL=1）→ exit 0；上午 47/47、下午 54/54、水平溢位=0；公開 CSS HTTP 200、54069 bytes、單一動畫控制規則存在；`08_HTML簡報` 未改投影片圖片 | 後續若要增加轉場效果，必須先通過全螢幕動畫 QA，不得讓投影片外層與內容層同時控制同一個 opacity／filter／transform |
| P1-11 | 已完成／公開可用 | 上午開場「降本增效」與下午 Human-in-the-loop 教學內容 | `node build_decks.mjs` → exit 0；PPTX 上午 47 頁、下午 54 頁；`node build_html_deck.mjs` → exit 0；HTML morning=47、afternoon=54；`node sync_dynamic_deck.mjs` → exit 0、version=`2026.08.23.05`；公開 `BASE_URL` QA → exit 0 且公開資料含新增文字 | 上午以開場標題／章節帶入「降本增效」；下午 Human-in-the-loop 實際仍為第 7 頁；圖檔版與 HTML 版均由同一來源重建 |
